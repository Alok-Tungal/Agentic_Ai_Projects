...
# --- 1. STARTUP EVENT ---
# This runs once when the user opens the page
@cl.on_chat_start
async def start():
    # Set up the model
    llm = ChatGoogleGenerativeAI(
    ...
    # Store the model in the user session so we can use it later
    cl.user_session.set("llm", llm)
    
    # Initialize chat history with the System Prompt
    system_prompt = "You are an expert AI Mentor. Be concise, helpful, and friendly."
    cl.user_session.set("history", [("system", system_prompt)])

    # Send a welcome message
    await cl.Message(
        content="👋 Hello! I am your AI Mentor. How can I help you with your coding today?"
    ).send()
...
async def main(message: cl.Message):
    # Retrieve the model and history from the session
    ...
    # Create an empty message to stream text into
    msg = cl.Message(content="")
    await msg.send()
    
    # Generate and stream the response
    # Chainlit handles the "typing" animation automatically
    full_response = ""
    ...
    # Update history with the full AI response
